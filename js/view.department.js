...
function renderHoseSelector() {
    ...
    wrapper.innerHTML = hoses.map(h => {
        ...
        const dia = h.diameter ? ` <span style="opacity:.75;">(${h.diameter}")</span>` : '';
        const cTxt = Number.isFinite(Number(h.c)) && Number(h.c) > 0 ? ` <span style="opacity:.75;">C ${Number(h.c)}</span>` : '';
        return `
    ...
    if (saveHosesBtn) {
        saveHosesBtn.onclick = () => {
            ...
            try {
                ...
                console.warn("saveDeptConfig hoses failed", e);
            }
