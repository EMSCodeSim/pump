...
import { getDeptNozzleLibrary } from "./deptNozzles.js";
// ------- DOM Helpers -------
    ...
    if (restoreDefaultsBtn) {
        restoreDefaultsBtn.onclick = () => {
            try {
                ...
                // Nozzles: default = all current nozzles
                const allNozzleIds = getDeptNozzleLibrary().map(n => n.id);
    ...
    if (restoreDefaultsBtn) {
        restoreDefaultsBtn.onclick = () => {
            try {
                ...
                // Nozzles: default = all entries from getDeptNozzleLibrary()
                const allNozzleIds = Array.isArray(getDeptNozzleLibrary())
                    ? getDeptNozzleLibrary().map(n => n.id)
                    : [];
                ...
                const uiList = Array.isArray(getDeptNozzleLibrary())
                    ? getDeptNozzleLibrary().map(n => ({
                        id: n.id,
                        label: n.label
                    }))
                    : [];
        ...
        if (deptHose && !document.getElementById('restore-dept-defaults')) {
            ...
            btn.addEventListener('click', () => {
                try {
                    ...
                    // Nozzles: reset to ALL nozzles in getDeptNozzleLibrary()
                    const allNozzleIds = getDeptNozzleLibrary().map(n => n.id);
