...
try{(function(){const s=document.createElement("style");s.textContent="@media (max-width: 420px){.kpis{flex-wrap:nowrap}.kpi b{font-size:16px}.kpi{padding:6px 8px}}";document.head.appendChild(s);}())}catch(e){}
 border-radius:12px;
    ...
    .presetsbtn.active,
    .supplybtn.active,
    .preset-line-btn.active {
      background:radial-gradient(circle at 10% 0%, #1e82ff, #1b3a72);
      border-color:#58b4ff;
      color:#f4f8ff;
      box-shadow:0 0 0 1px rgba(70,176,255,.3),0 10px 24px rgba(0,0,0,.6);
    }
...
// Presets button: wire to full preset system even on PC/web
  ...
  function drawAll(){
    ...
    // Line button active states (Line 1/2/3)
    ['left','back','right'].forEach(key => {
      const L = state.lines[key];
      const btn = container.querySelector(`.linebtn[data-line="${key}"]`);
      if (btn && L) {
    ...
    if (presetsBtn) {
      const anyPresetLines = activePresetLines && Object.keys(activePresetLines).length > 0;
      presetsBtn.classList.toggle('active', anyPresetLines);
    }
