// view.calc.js
// Lines UI (drawing, presets, “+” tip editor, Why? math with in-panel mini graphics).
...
function addLabel(G_labels, text, x, y){
...
function findNozzle(targetGpm, targetNP){
  ...
  for(const n of NOZ_LIST){
    const e = Math.abs((n.gpm||0)-targetGpm) + 2*Math.abs((n.NP||0)-targetNP);
...
function mathGraphicSVG(calc){
  ...
  const parts = [];
  if(calc.NP)   parts.push({k:'NP',   v:calc.NP,   col:SEG_COLOR.np});
  if(calc.FLm)  parts.push({k:'FLm',  v:calc.FLm,  col:SEG_COLOR.flm});
  if(calc.FLb)  parts.push({k:'FLb',  v:calc.FLb,  col:SEG_COLOR.flb});
  if(calc.WYE)  parts.push({k:'Wye',  v:calc.WYE,  col:SEG_COLOR.wye});
  if(calc.ELEV) parts.push({k:'Elev', v:calc.ELEV, col:SEG_COLOR.elev});

  let cx = pad;
  const bars = parts.map(p=>{
    const w = Math.max(2, Math.round((p.v/total)*bw));
    const x = cx; cx += w + 1;
    return `<rect x="${x}" y="${pad+2}" width="${w}" height="${bh}" fill="${p.col}"></rect>`;
  }).join('');
...
export async function render(container){
      ...
      <section class="card">
        ...
        <div id="hydrantHelper" class="helperPanel" style="display:none; margin-top:10px; background:#0e151e; border:1px solid rgba(255,255,255,.1); border-radius:12px; padding:12px;">
          ...
          <div class="row" style="display:flex; gap:10px; flex-wrap:wrap">
            <div class="field" style="min-width:140px"><label>Static (psi)</label><input type="number" id="hydrantStatic" placeholder="80" inputmode="decimal"></div>
            <div class="field" style="min-width:170px"><label>Residual w/ 1 line (psi)</label><input type="number" id="hydrantResidual" placeholder="72" inputmode="decimal"></div>
            <div class="field" style="min-width:150px; display:flex; align-items:flex-end"><button class="btn primary" id="hydrantCalcBtn" type="button">Evaluate %Drop</button></div>
          </div>
        ...
        <div id="staticHelper" class="helperPanel" style="display:none; margin-top:10px; background:#0e151e; border:1px solid rgba(255,255,255,.1); border-radius:12px; padding:12px;">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
            <div><div style="color:#fff; font-weight:800;">Tender Shuttle (Static Supply)</div>
          ...
          <div class="row" style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;">
            <div class="field" style="min-width:160px"><label>Tender ID / Number</label><input id="tAddId" type="text" placeholder="Tender 2"></div>
            <div class="field" style="min-width:160px"><label>Capacity (gal)</label><input id="tAddCap" type="number" inputmode="decimal" placeholder="3000"></div>
            <div class="field" style="min-width:140px; display:flex; align-items:flex-end"><button id="tAddBtn" class="btn primary" type="button">Add Tender</button></div>
          </div>
  ...
  injectStyle(container, `
    ...
    .ppRow .hdr{font-weight:800;margin-bottom:6px; display:flex; align-items:center; gap:8px; flex-wrap:wrap}
  ...
  function applyPresetTo(preset, key){
    ...
    switch(preset){
      case 'standpipe': L.itemsMain=[{size:'2.5', lengthFt:0}];   L.nozRight=findNozzle(150,75)||L.nozRight; L.hasWye=false; L.elevFt=60; break;
      case 'sprinkler': state.supply='pressurized'; L.itemsMain=[{size:'2.5', lengthFt:50}]; L.nozRight=findNozzle(150,75)||L.nozRight; L.hasWye=false; break;
      case 'foam':      L.itemsMain=[{size:'1.75', lengthFt:200}]; L.nozRight=findNozzle(185,50)||L.nozRight; L.hasWye=false; L.elevFt=0; break;
      case 'monitor':   L.itemsMain=[{size:'2.5', lengthFt:200}];  L.nozRight=findNozzle(265,50)||L.nozRight; L.hasWye=false; L.elevFt=0; break;
      case 'aerial':    state.supply='pressurized'; L.itemsMain=[{size:'2.5', lengthFt:150}]; L.nozRight=findNozzle(265,50)||L.nozRight; L.hasWye=false; L.elevFt=80; break;
    }
    drawAll();
  }
  ...
  container.querySelector('#teApply').addEventListener('click', ()=>{
    ...
    if(wantWye){
      ...
      const nzA = NOZ_LIST.find(n => n.id === teNozA.value) || L.nozLeft || findNozzle(185,50) || NOZ_LIST?.[0];
      const nzB = NOZ_LIST.find(n => n.id === teNozB.value) || L.nozRight || findNozzle(185,50) || NOZ_LIST?.[0];
      L.nozLeft  = nzA; L.nozRight = nzB;
      L.wyeLoss = L.wyeLoss || 10;
    } else {
      L.itemsLeft=[]; L.itemsRight=[];
      const nz = NOZ_LIST.find(n => n.id === teNoz.value) || L.nozRight || findNozzle(185,50) || NOZ_LIST?.[0];
      L.nozRight = nz;
    }
  ...
  function calcLine(L){
    ...
    let NP=0, FLb=0, PDP=0, WYE=0, ELEV=(L.elevFt||0)*PSI_PER_FT;
    ...
    if(single){
      ...
      NP = (bnNoz?.NP||0);
      FLb = FL_total(bnNoz?.gpm||0, bnSegs);
      PDP = NP + FLb + mainFL + ELEV;
    } else if(L.hasWye){
    ...
    return { flow, NP, FLm:mainFL, FLb, WYE, ELEV, PDP };
  }
  ...
  function refreshTotals(){
    ...
    vis.forEach(([key, L])=>{
      const c = calcLine(L);
      totalGPM += c.flow;
      if(c.PDP > maxPDP){ maxPDP = c.PDP; maxKey = key; }
    });
  ...
  function renderLinesPanel(){
    ...
    keys.forEach(k=>{
      ...
      if(L.hasWye){
        ...
        rows.push(`Wye appliance loss = <b>${Math.round(c.WYE)}</b> psi`);
        ...
        rows.push(`Main FL (${c.flow} gpm) = <b>${Math.round(c.FLm)}</b> psi`);
        rows.push(`Elevation (${(L.elevFt||0)}′) = <b>${Math.round(c.ELEV)}</b> psi`);
        rows.push(`<i>Active branch:</i> <b>${side==='L'?'Left':'Right'}</b>`);
      } else {
        rows.push(`Nozzle (${L.nozRight?.name||'—'}) NP = <b>${Math.round(c.NP)}</b> psi`);
        rows.push(`Main FL (${c.flow} gpm) = <b>${Math.round(c.FLm)}</b> psi`);
        rows.push(`Elevation (${(L.elevFt||0)}′) = <b>${Math.round(c.ELEV)}</b> psi`);
      }
      ...
      html += `
        <div class="ppRow" id="pp_simple_${k}">
          <div class="hdr">
            ${L.label} — PDP <b>${Math.round(c.PDP)}</b> psi
            ${mathGraphicSVG(c)}
          </div>
  ...
  function drawAll(){
    ...
    clearGroup(G_hoses); clearGroup(G_branches); clearGroup(G_labels); clearGroup(G_tips); clearGroup(G_supply);
    ...
    visibleKeys.forEach(key=>{
      ...
      path.setAttribute('class', clsForSize(mainSize));
      ...
      if(L.hasWye){
        if(sumFt(L.itemsLeft)>0){
          ...
          pL.setAttribute('class', clsForSize(sizeL)+' branch');
        ...
        if(sumFt(L.itemsRight)>0){
          ...
          pR.setAttribute('class', clsForSize(sizeR)+' branch');
