...
function ppExplainHTML(L){
  ...
  if(!L.hasWye){
    return `
      <div><b>Simple PP:</b>
        <ul class="simpleList">
          ...
          <li><b>Elevation</b> = ${(side==='L' ? ((L.elevLeftFt||0)*PSI_PER_FT) : ((L.elevRightFt||0)*PSI_PER_FT))>=0? '+':'')+fmt((side==='L' ? ((L.elevLeftFt||0)*PSI_PER_FT) : ((L.elevRightFt||0)*PSI_PER_FT)))+' psi'}</li>
        </ul>
    ...
    const elevPsiBn = ((side==='L' ? (L.elevLeftFt||0) : (L.elevRightFt||0)) * PSI_PER_FT);
    const total = noz.NP + brSum + mainSum + elevPsiBn;
    return `
    ...
    const elevPsiMax = (Math.max(L.elevLeftFt||0, L.elevRightFt||0) * PSI_PER_FT);
    const total = maxNeed + mainSum + wyeLoss + elevPsiMax;
    return `
      <div><b>Simple PP (Wye):</b>
        <ul class="simpleList">
          ...
          <li><b>Elevation</b> = ${(elevPsiMax>=0? '+':'')+fmt(elevPsiMax)+' psi'}</li>
        </ul>
