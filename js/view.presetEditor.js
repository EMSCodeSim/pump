...
// Preset line editor popup (name + type selection) wired to all builder popups.
...
export function openPresetEditorPopup(options) {
  ...
  function openTypeBuilder(id, label) {
    if (id === "standard") {
      openStandardLinePopup({
        dept,
        initial: state.configs.standard,
        onSave(config) {
          state.configs.standard = config;
          updatePreview();
        }
      });
    } else if (id === "master") {
      ...
      openSprinklerPopup({
        ...
        onSave(config) {
          state.configs.sprinkler = config;
          updatePreview();
        }
      });
    } else if (id === "foam") {
      ...
      openCustomBuilderPopup({
        ...
        onSave(config) {
          state.configs.custom = config;
          updatePreview();
        }
      });
    } else {
      alert('Preset type "' + label + '" selected, but no builder is wired for this type.');
    }
  }
  ...
  saveBtn.addEventListener("click", function (e) {
    ...
    onSave({
      ...
      // specific fields (easier to use later in preset.js / view.calc)
      ...
      // also include full map for flexibility
